...
export default class OrderPointComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('order-point-heading'));
...
export class OrderPointDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('borditApp.orderPoint.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-orderPoint'));
}
